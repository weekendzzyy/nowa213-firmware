...
> **⚠️ 勘误（2026-09-11 补记）**：本笔记写于 2026-09-03，当时据官方手册一处被截断的文本
> 判断屏驱为 **SSD1675**，**这个判断是错的**。后续实机确认：
>
> - 屏幕左上角自报 `ESL_xxxxxx BWR213` ⇒ 固件 `epd_model == 2` ⇒ 实际执行的是
>   `epd_bwr_213.c`，该驱动面向 **SSD1680** 族（其文件头也写着 "SSD1675 mixed with
>   SSD1680 EPD Controller"）。
> - 应查的手册版本是 **SSD1680 Rev 0.14**，不是 SSD1675。
> - **下文所有 `SSD1675` 字样请按 `SSD1680` 理解。** 原文保留不改，是为了留下当时的
>   判断过程 —— 这次误判的代价很实在：v6.0 把局部刷新写进了**永不执行**的兜底驱动
>   `epd_bw_213.c`，整版白做。
>
> 现行结论与正确的改动入口见 [`DEVELOPMENT.md` §4.2](../DEVELOPMENT.md)。
