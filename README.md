...
把汉朔（Hanshow）**Nowa-213R-N** 2.13" 三色电子价签改造成**三页显示的低功耗固件**
（时钟 / 月历 / 图片），手机 **NFC 贴一下翻页**，并持续压低它的刷新能耗。
...
![屏幕预览](docs/images/screen-v15.2.png)
...
当前 **v15.2**。完整历史与每个发布固件的 SHA256 见 [CHANGELOG.md](CHANGELOG.md)。
...
| **v15.0** | **09-13** | **三页显示**：时间 / 月历（周末红 + 今日反色 + 放大红字）/ 图片；**启用红色层**（RAM 零增量）；BLE opcode 0x05/0x06 切页 |
| **v15.1** | **09-13** | **NFC 贴一下翻页**：FM11NC081 场检测（PC4 唤醒 + 去抖 + 等待释放）；蓝牙电源模式未动，BLE 仍常开 |
| **v15.2** | **09-13** | **NFC 命令通道**：手机写帧即可校时(0x04)/切页(0x01/0x02)/强制全刷(0x05)/读状态；双形态（裸字节+ASCII 十六进制文本）；蓝牙电源仍一行未动（省电模式 §5.1 铁律拒绝） |
...
```
┌──────────────────────────────────────────┐
│ 2026年9月12日 周六 31℃ 2905mV            │  行1 Unifont 16
│                                          │
...
- **名字照参考图缩成 4 位十六进制**（广播名 `ESL_A1B2C3` 的末 4 位）。参考面板显示的
  `[881A]` 正是这个形式；4 位 = 16 bit，区分设备绰绰有余。
- **右下角整体收进局部刷新带内**（右对齐到玻璃 x 231 = 带的最后一列）。这不是审美：
  栅极带以外的像素在局部刷新时**保持原样**，如果名字/版本跨出带外，每次切换都会留下
  旧串的尾巴直到整点全刷。收进带内之后，切换由本来就发生的分钟局部刷新顺手完成，
  **零额外全刷**。副作用是行 3 与行 1 的右边缘对齐了（行 1 最宽止于 228）。
- 符文槽是**固定**的（172..179），不跟随文字左缘 —— 两个串宽度不同，跟着走符号就会
  每 5 分钟横跳 8 px。

三件事值得单独说：

1. **大时钟是 DSEG14 字体（数码管），放大 1.5 倍到 60 px。** v14.0/v14.1 曾用手绘
   7 段去追参考照片的 68 px 高度，用户看了实机后不要 —— 换回了 v13.0 的 `DSEG14`。
   位图字体放大只能整数倍：2 倍是 290 px（超宽），所以是 **3/2 最近邻**（源像素按
   2,1,2,1 展开成 1.5 倍，笔画宽度误差 ≤1 px）。字体经 `gen_v14_tables.py` 从上游
   93 字形裁到时钟用的 11 个（0-9 与冒号），位图 1.2 KB。镜像 82468 → **82864 B**。
   温度也一并修了（见 CHANGELOG）：v14.0/v14.1 屏上恒显 85 ℃，因为读的是 ADC 原始
   采样值而不是摄氏度。
