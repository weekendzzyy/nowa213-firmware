...
![屏幕预览](docs/images/screen-v14.2.png)
...
![参考面板与实现对照](docs/images/reference-vs-v14.2.png)
...
当前 **v14.2**。完整历史与每个发布固件的 SHA256 见 [CHANGELOG.md](CHANGELOG.md)。
...
│                                          │
│ 八月初二 11天后秋分    ⚡ [B2A1] / v14.2  │  行3 符文槽 172..179（固定）
└──────────────────────────────────────────┘
       ↑ 局部刷新窗口 玻璃 x 140..231 = 栅极 187..278（92/296 = 31%）
```

行 3 右下角在**广播名 `[B2A1]` 与版本号 `v14.1` 之间每 5 分钟交替**（`ROW3_ALT_SECS`）。
两个设计决定值得说明：
...
python tools/verify_v14_layout.py        # 期望 "75 checks passed, 0 failed"
...
| `verify_v14_layout.py` | 75 项断言：格子边界、窗口=实际变化列的精确并集（枚举全部 1440 个 HH:MM 逐拍 diff）、行 1 最宽串、农历 9131 天逐日走查、以及**否定性自检**（把每个已修 bug 重新植入，确认对应断言真的会 FAIL）|
