...
![屏幕预览](docs/images/screen-v12.0.png)
...
当前 **v12.0**。完整历史与每个发布固件的 SHA256 见 [CHANGELOG.md](CHANGELOG.md)。
...
> **v12.0 是临时诊断固件。** 屏上会多一串 `H%d T%d B%d L%d`
> （小时 / 温度 / 电池 / 蓝牙各自触发全刷的次数），刷新行为与 v11.0 完全一致。
> 定位完问题后把 `epd.c` 的 `EPD_USE_REFRESH_DEBUG` 置 0 即可一键摘除。
